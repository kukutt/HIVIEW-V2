...
int sample_system_init(){
	return sample_system_initV2(SENSOR_NAME, SENSOR_CUBS_TYPE, SENSOR_I2C_ADDR);
}

int sample_system_initV2(const char *snsname, IMPSensorControlBusType type, int addr)
{
	int ret = 0;
	...
	memcpy(sensor_info.name, snsname, strlen(snsname));
	sensor_info.cbus_type = type;
	memcpy(sensor_info.i2c.type, snsname, strlen(snsname));
	sensor_info.i2c.addr = addr;
...
static void *get_video_stream(void *args)
{
  ...
  for (i = 0; i < totalSaveCnt; i++) {
    ...
    if (encType == IMP_ENC_TYPE_JPEG) {
      ...
      if (chnNum == 0){
        printf("[%d/%d] count=%d pts=%lld time=%ld\r\n", i, totalSaveCnt, stream.packCount, stream.pack[0].timestamp, time(NULL));
      }
